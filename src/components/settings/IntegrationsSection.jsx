...
const INTEGRATIONS = [
  { id: "google", name: "Google Calendar", icon: FaGoogle, color: "text-blue-400",   bg: "bg-blue-500/10",   connected: false },
  { id: "slack",  name: "Slack",           icon: FaSlack,  color: "text-purple-400", bg: "bg-purple-500/10", connected: false },
  { id: "github", name: "GitHub",          icon: FaGithub, color: "text-zinc-300",   bg: "bg-white/[0.05]",  connected: false },
  { id: "notion", name: "Notion",          icon: SiNotion, color: "text-orange-400", bg: "bg-orange-500/10", connected: false },
  { id: "asana",  name: "Asana",           icon: SiAsana,  color: "text-teal-400",   bg: "bg-teal-500/10",   connected: false },
  { id: "linear", name: "Linear",          icon: SiLinear, color: "text-indigo-400", bg: "bg-indigo-500/10", connected: false },
];
...
function IntegrationsSection() {
  ...
  const handleConnect = (id, name) => {
    addNotification({
      ...
      message: "OAuth integrations are coming soon.",
      type:    "info",
    });
  };
  ...
  const handleDisconnect = (id, name) => {
    ...
    addNotification({
      ...
      message: `${name} has been disconnected from your account.`,
      type:    "info",
    });
  };
        ...
        {INTEGRATIONS.map(({ id, name, icon: Icon, color, bg }) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className={`mt-0.5 text-xs ${states[id] ? "text-emerald-400" : "text-zinc-600"}`}>
